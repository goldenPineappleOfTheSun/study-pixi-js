...
const skeletonFunctions = {
            ...
            if (!svision.check({...pointA, ...{width: 1, height: 1}})) {
                for (let i=0; i<CELL_HEIGHT * 1.5; i+=8) {
                    if (svision.check({x:pointA.x, y:pointA.y+i, width:1, height:1})) {
                        const jumpHeight = (CELL_HEIGHT * 1.5 - i)
                        if (this.doesHaveSpaceAbove(jumpHeight + bounds.height)) {
                            this.runScript('jump', true, jumpHeight * 2.5, jumpHeight * 7, 1, jumpHeight * 1);
                            jumpHappened = true;
                            break;
                        }
                    }
    ...
    */
    ...
    step(delta) {
        ...
        /* linecast showcase */
        this.line.clear();
        this.line.position.set(hero.x, hero.y);
        this.line.position.set(this.x, this.y);
        this.line.lineStyle(1, 0x000000)
           .moveTo(0, 0)
           .lineTo(hero.x - this.x, hero.y - this.y);

        for (let block of stage.layers.default.children) {
            if (block.tag !== 'block') {
                continue;
            }
            let line = {x1:this.x, y1:this.y, x2:hero.x, y2:hero.y};
            let a = getLinesIntersection(line, {x1:block.x-32, y1:block.y-32, x2:block.x+32, y2:block.y-32});
            let b = getLinesIntersection(line, {x1:block.x+32, y1:block.y-32, x2:block.x+32, y2:block.y+32});
            let c = getLinesIntersection(line, {x1:block.x-32, y1:block.y+32, x2:block.x+32, y2:block.y+32});
            let d = getLinesIntersection(line, {x1:block.x-32, y1:block.y-32, x2:block.x-32, y2:block.y+32});

            const renderPoint = (p) => {return {x:p.x-5, y:p.y-5, width:10, height:10}};
            if (a) {debug(renderPoint(a), 'red')}
            if (b) {debug(renderPoint(b), 'lime')}
            if (c) {debug(renderPoint(c), 'blue')}
            if (d) {debug(renderPoint(d), 'purple')}
        }
    }
}
...
function createSkeleton(x, y) {
    ...
    /* linecast showcase */
    container.line = new PIXI.Graphics();
    app.stage.addChild(container.line);
